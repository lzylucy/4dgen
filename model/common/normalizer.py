...
from model.common.dict_of_tensor_mixin import (
    DictOfTensorMixin,
)
...
class SingleFieldLinearNormalizer(DictOfTensorMixin):
    ...
    @classmethod
    def create_fit(
        cls, data: Union[torch.Tensor, np.ndarray, zarr.Array], **kwargs
    ):
        obj = cls()
        obj.fit(data, **kwargs)
        return obj
...
def _fit(
    ...
    if mode == "limits":
        if fit_offset:
            ...
            input_abs = torch.maximum(
                torch.abs(input_min), torch.abs(input_max)
            )
...
def create_normalizer_with_identity(shape_meta: Dict) -> LinearNormalizer:
    camera_names = []
    lowdim_names = []
    ...
    for key, attr in shape_meta["obs"].items():
        ...
        if type == "rgb":
            ...
            lowdim_names.append(key)
        else:
            assert False

    normalizer = LinearNormalizer()
    for name in lowdim_names + ["action"]:

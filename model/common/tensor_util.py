...
def recursive_dict_list_tuple_apply(x, type_func_dict):
    """
    Recursively apply functions to a nested dictionary or list or tuple, given
    a dictionary of {data_type: function_to_apply}.

    Args:
        x (dict or list or tuple): a possibly nested dictionary or list or
            tuple
...
def to_batch(x):
    ...
    arrays in nested dictionary or list or tuple and returns a new nested
    structure.
...
def to_sequence(x):
    """
    Introduces a time dimension of 1 at dimension 1 for all torch tensors and
    numpy arrays in nested dictionary or list or tuple and returns a new nested
    structure.
...
def index_at_time(x, ind):
    """
    Indexes all torch tensors and numpy arrays in dimension 1 with index @ind
...
def unsqueeze(x, dim):
    """
    Adds dimension of size 1 at dimension @dim in all torch tensors and numpy
    arrays in nested dictionary or list or tuple and returns a new nested
    structure.
...
def to_torch(x, device):
    """
    Converts all numpy arrays and torch tensors in nested dictionary or list or
    tuple to torch tensors on device @device and returns a new nested
    structure.
...
def to_one_hot_single(tensor, num_class):
    """
    Convert tensor to one-hot representation, assuming a certain number of
    total class labels.
...
def to_one_hot(tensor, num_class):
    """
    Convert all tensors in nested dictionary or list or tuple to one-hot
    representation, assuming a certain number of total class labels.

    Args:
        tensor (dict or list or tuple): a possibly nested dictionary or list or
            tuple
        num_class (int): number of classes
    ...
    """
    return map_tensor(
        tensor, func=lambda x, nc=num_class: to_one_hot_single(x, nc)
    )
...
def flatten(x, begin_axis=1):
    """
    Flatten all tensors in nested dictionary or list or tuple, from @begin_axis
    onwards.
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            torch.Tensor: lambda x, b=begin_axis: flatten_single(
                x, begin_axis=b
            ),
        },
    )
...
def reshape_dimensions(x, begin_axis, end_axis, target_dims):
    """
    Reshape selected dimensions for all tensors in nested dictionary or list or
    tuple to a target dimension.
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            torch.Tensor: lambda x, b=begin_axis, e=end_axis, t=target_dims: reshape_dimensions_single(  # noqa
                x, begin_axis=b, end_axis=e, target_dims=t
            ),
            np.ndarray: lambda x, b=begin_axis, e=end_axis, t=target_dims: reshape_dimensions_single(  # noqa
                x, begin_axis=b, end_axis=e, target_dims=t
            ),
            type(None): lambda x: x,
        },
...
def join_dimensions(x, begin_axis, end_axis):
    """
    Joins all dimensions between dimensions (@begin_axis, @end_axis) into a
    flat dimension, for all tensors in nested dictionary or list or tuple.
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            torch.Tensor: lambda x, b=begin_axis, e=end_axis: reshape_dimensions_single(  # noqa
                x, begin_axis=b, end_axis=e, target_dims=[-1]
            ),
            np.ndarray: lambda x, b=begin_axis, e=end_axis: reshape_dimensions_single(  # noqa
                x, begin_axis=b, end_axis=e, target_dims=[-1]
            ),
            type(None): lambda x: x,
        },
...
def gather_along_dim_with_dim_single(x, target_dim, source_dim, indices):
    """
    This function indexes out a target dimension of a tensor in a structured
    way, by allowing a different value to be selected for each member of a flat
    index tensor (@indices) corresponding to a source dimension. This can be
    interpreted as moving along the source dimension, using the corresponding
    index value in @indices to select values for all other dimensions outside
    of the source and target dimensions. A common use case is to gather values
    ...
    Args:
        ...
        source_dim (int): dimension to hold constant and use for gathering
            values from the other dimensions
        indices (torch.Tensor): flat index tensor with same shape as tensor @x
            along @source_dim

    Returns:
        y (torch.Tensor): gathered tensor, with dimension @target_dim indexed
            out
    """
...
def gather_along_dim_with_dim(x, target_dim, source_dim, indices):
    ...
    return map_tensor(
        x,
        lambda y, t=target_dim, s=source_dim, i=indices: gather_along_dim_with_dim_single(  # noqa
            y, t, s, i
        ),
    )
...
def gather_sequence_single(seq, indices):
    """
    Given a tensor with leading dimensions [B, T, ...], gather an element from
    each sequence in the batch given an index for each sequence.
...
def gather_sequence(seq, indices):
    """
    Given a nested dictionary or list or tuple, gathers an element from each
    sequence of the batch for tensors with leading dimensions [B, T, ...].
    ...
    Returns:
        y (dict or list or tuple): new nested dict-list-tuple with tensors of
            shape [B, ...]
    """
    return gather_along_dim_with_dim(
        seq, target_dim=1, source_dim=0, indices=indices
    )
...
def pad_sequence_single(
    ...
    Args:
        ...
        pad_values (scalar or (ndarray, Tensor)): values to be padded if not
            pad_same
...
def pad_sequence(seq, padding, batched=False, pad_same=True, pad_values=None):
    ...
    return recursive_dict_list_tuple_apply(
        seq,
        {
            torch.Tensor: lambda x, p=padding, b=batched, ps=pad_same, pv=pad_values: pad_sequence_single(  # noqa
                x, p, b, ps, pv
            ),
            np.ndarray: lambda x, p=padding, b=batched, ps=pad_same, pv=pad_values: pad_sequence_single(  # noqa
                x, p, b, ps, pv
            ),
            type(None): lambda x: x,
        },
...
def get_shape(x):
    ...
    Returns:
        y (dict or list or tuple): new nested dict-list-tuple that contains
            each array or tensor's shape
    """
    return recursive_dict_list_tuple_apply(
        x,
        {
...
def list_of_flat_dict_to_dict_of_list(list_of_dict):
    """
    Helper function to go from a list of flat dictionaries to a dictionary of
    lists. By "flat" we mean that none of the values are dictionaries, but are
    numpy arrays, floats, etc.
...
def flatten_nested_dict_list(d, parent_key="", sep="_", item_key=""):
    ...
    if isinstance(d, (tuple, list)):
        new_key = (
            parent_key + sep + item_key if len(parent_key) > 0 else item_key
        )
...
def time_distributed(
    ...
    **kwargs
    ...
    """
    Apply function @op to all tensors in nested dictionary or list or tuple
    @inputs in both the batch (B) and time (T) dimension, where the tensors are
    expected to have shape [B, T, ...]. Will do this by reshaping tensors to
    [B * T, ...], passing through the op, and then reshaping outputs to
    [B, T, ...].

    Args:
        inputs (list or tuple or dict): a possibly nested dictionary or list or
            tuple with tensors of leading dimensions [B, T, ...]
        op: a layer op that accepts inputs
        activation: activation to apply at the output
        inputs_as_kwargs (bool): whether to feed input as a kwargs dict to the
            op
        inputs_as_args (bool) whether to feed input as a args list to the op
        kwargs (dict): other kwargs to supply to the op

    Returns:
        outputs (dict or list or tuple): new nested dict-list-tuple with
            tensors of leading dimension [B, T].
    """

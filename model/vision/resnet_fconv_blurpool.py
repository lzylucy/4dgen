# This code is built from the PyTorch examples repository:
...
# Adobe’s modifications are licensed under the Creative Commons Attribution-NonCommercial-ShareAlike  # noqa
# 4.0 International Public License (CC-NC-SA-4.0). To view a copy of the license, visit  # noqa
# https://creativecommons.org/licenses/by-nc-sa/4.0/legalcode.
# =============================================================================
# This implementation includes Same-Conv, BlurPool, Full-Conv and Full-Conv
# with BlurPool versions all together.
# For more details see the paper:
# O. S. Kayhan and J. van Gemert,
# "On Translation Invariance in CNNs: Convolutional Layers can Exploit Absolute Spatial Location"  # noqa
# In CVPR, 2020.
...
from model.vision.blurpool import BlurPool as Downsample
...
class Downsample(nn.Module):
    def __init__(
        self, pad_type="zero", filt_size=3, stride=2, channels=None, pad_off=0
    ):  # reflect
    ...
    def __init__(
        ...
        if stride == 1:
            ...
            self.conv2 = nn.Sequential(
                Downsample(
                    filt_size=filter_size, stride=stride, channels=planes
                ),
        ...
        self.zero_pad_Id = nn.ZeroPad2d(
            3
        )  # padding on Identity while downsampling
    ...
    def __init__(
        ...
        if pool_only:
            self.conv1 = nn.Conv2d(
                3, planes[0], kernel_size=7, stride=2, padding=6, bias=False
            )
        else:
            self.conv1 = nn.Conv2d(
                3, planes[0], kernel_size=7, stride=1, padding=6, bias=False
            )
        ...
        if pool_only:
            self.maxpool = nn.Sequential(
                *[
                    ...
                    Downsample(
                        filt_size=filter_size, stride=2, channels=planes[0]
                    ),
                ]
            )
        else:
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                if (
                    ...
                    # assuming normal conv layers will not have these
                    # characteristics
...
def resnext101_32x8d(
    pretrained=False, filter_size=1, pool_only=True, **kwargs
):
    model = ResNet(
        Bottleneck,
        [3, 4, 23, 3],

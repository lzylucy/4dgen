"""
All copied from https://github.com/ARISE-Initiative/robomimic/blob/b5d2aa9902825c6c652e3b08b19446d199b49590/robomimic/models/base_nets.py#L45  # noqa
"""
...
class Module(torch.nn.Module):
    """
    Base class for networks. The only difference from torch.nn.Module is that
    it requires implementing @output_shape.
    """
...
class ConvBase(Module):
    ...
    # dirty hack - re-implement to pass the buck onto subclasses from ABC
    # parent
    ...
    def forward(self, inputs):
        x = self.nets(inputs)
        if (
            list(self.output_shape(list(inputs.shape)[1:]))
            != list(x.shape)[1:]
        ):
            raise ValueError(
...
class SpatialSoftmax(ConvBase):
    ...
    def __init__(
        ...
        Args:
            ...
            output_variance (bool): treat attention as a distribution, and
                compute second-order statistics to return
            noise_std (float): add random spatial noise to the predicted
                keypoints
        """
        ...
        pos_x = torch.from_numpy(
            pos_x.reshape(1, self._in_h * self._in_w)
        ).float()
        pos_y = torch.from_numpy(
            pos_y.reshape(1, self._in_h * self._in_w)
        ).float()
    ...
    def forward(self, feature):
        ...
        Returns:
            out (torch.Tensor or tuple): mean keypoints of shape [B, K, 2], and
                possibly keypoint variance of shape [B, K, 2, 2] corresponding
                to the covariance under the 2D spatial softmax distribution
        """
        ...
        # [1, H * W] x [B * K, H * W] -> [B * K, 1] for spatial coordinate mean
        ...
        if self.output_variance:
            ...
            # stack to [B * K, 4] and then reshape to [B, K, 2, 2] where last 2
            # dims are covariance matrix
            feature_covar = torch.cat(
                [var_x, var_xy, var_xy, var_y], 1
            ).reshape(-1, self._num_kp, 2, 2)
            feature_keypoints = (feature_keypoints, feature_covar)

        if isinstance(feature_keypoints, tuple):
            self.kps = (
                feature_keypoints[0].detach(),

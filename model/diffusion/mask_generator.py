    ...
    def forward(self, shape, seed=None):
        ...
        if self.fix_obs_steps:
            obs_steps = torch.full(
                (B,), fill_value=self.max_n_obs_steps, device=device
            )
        else:
        ...
        if self.action_visible:
            action_steps = torch.maximum(
                obs_steps - 1,
                torch.tensor(
                    0, dtype=obs_steps.dtype, device=obs_steps.device
                ),
            )
            action_mask = (
...
class KeypointMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        ...
        if self.time_independent:
            visible_kps = (
                torch.rand(
                    size=(B, T, n_keypoints), generator=rng, device=device
                )
            ...
            visible_dims_mask = torch.cat(
                [
                    torch.ones(
                        (B, self.action_dim), dtype=torch.bool, device=device
                    ),
                    visible_dims,
                    torch.ones(
                        (B, self.context_dim), dtype=torch.bool, device=device
                    ),
                ],
                axis=-1,
            )

...
class AbstractAutoencoder(pl.LightningModule):
    ...
    def __init__(
        ...
        input_key: str = "jpg",
        input_mv_keys: Union[None, List[str]] = None,
    ):
        super().__init__()
    ...
    def apply_ckpt(self, ckpt: Union[None, str, dict]):
        if ckpt is None:
            # NOTE: it is normal to get these warnings when finetuning because
            # the ckpt is loaded in diffusion.py instead of here.
            print("Autoencoder ckpt is None, skipping!")
            return

        elif isinstance(ckpt, str):
            # Needed to refactor this because CheckpointEngine is not provided.

            print("Autoencoder ckpt is not None...")
            ...
            # assert unexpected_keys == ['model_ema.decay', 'model_ema.num_updates']
...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def get_mv_input(self, batch):
        ...
        return batch[self.input_mv_keys[np.random.choice(len(self.input_mv_keys))]]
    ...
    def inner_training_step(
        ...
        additional_decode_kwargs['timesteps'] = batch['num_video_frames'][0]
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> Dict:
        ...
        log_image_dict = self.log_images(batch)
        log_dict.update(log_image_dict)
        return log_dict

        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    v1 = batch[embedder.input_keys[0]]
                    v2 = batch[embedder.input_keys[1]]
                    interleaved = torch.stack((v1, v2), dim=1).reshape(2 * v1.shape[0], *v1.shape[1:])
                    emb_out = embedder(interleaved)
            assert isinstance(
            ...
            for emb in emb_out:
                ...
                if (
                    ...
                    and set(embedder.input_keys) <= set(force_zero_embeddings)
                ):
                    emb = torch.zeros_like(emb)

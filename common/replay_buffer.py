...
def get_optimal_chunks(
    shape, dtype, target_chunk_bytes=2e6, max_chunk_length=None
):
    """
    Common shapes
    T,D
...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        """
        Dummy constructor. Use copy_from* and create_from* class methods
        instead.
        """
        assert "data" in root
        assert "meta" in root
        ...
        **kwargs,
        ...
        for key, value in self.root["data"].items():
            ...
            if isinstance(value, zarr.Array):
                ...
                # numpy
                _ = data_group.array(
                    name=key, data=value, chunks=cks, compressor=cpr
                )
        return store
    ...
    @classmethod
    def create_from_s3(cls, s3_path, cache_size=2**28):
        """
        cache_size: number of bytes
        """
        s3 = s3fs.S3FileSystem()
        store = s3fs.S3Map(root=s3_path, s3=s3, check=False)
        cache = zarr.LRUStoreCache(store, max_size=cache_size)
        root = zarr.group(store=cache)
        return cls.create_from_group(root)
    ...
    def episode_paths(self):
        return self.meta.get("episode_paths", None)
    ...
    def add_episode(
        ...
        *,
        ...
        # append to episode paths
        if path is not None:
            if is_zarr:
                if "episode_paths" not in self.meta:
                    self.meta.create("episode_paths", shape=(1,), dtype="str")
                else:
                    self.episode_paths.resize(self.episode_paths.shape[0] + 1)

                self.episode_paths[-1] = path
            else:
                pass
        ...
        if is_zarr:
            ...
            if episode_paths is not None:
                if episode_paths.chunks[0] < episode_paths.shape[0]:
                    rechunk_recompress_array(
                        self.meta,
                        "episode_paths",
                        chunk_length=int(episode_paths.shape[0] * 1.5),
                    )

    def drop_episode(self):
        """
        Removes the last episode
        """
        is_zarr = self.backend == "zarr"
        ...
        if is_zarr:
            ...
            if self.episode_paths is not None:
                self.episode_paths.resize(len(episode_ends) - 1)
        else:
            self.episode_ends.resize(len(episode_ends) - 1, refcheck=False)
            if self.episode_paths is not None:
                self.episode_paths.resize(
                    len(episode_ends) - 1, refcheck=False
                )
        ...
        for key, value in compressors.items():
            if key in self.data:
                ...
                if compressor != arr.compressor:
                    rechunk_recompress_array(
                        self.data, key, compressor=compressor
                    )
